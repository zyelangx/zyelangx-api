...
router.get('/downloader/ytmp4', async (req, res, next) => {
    ...
    if (!url) {
        res.status(403).json(logger.noturl)
        return
    }
    await axios.get('https://saipulanuar.ga/api/download/ytmp4?url=' + url)
        .then((res) => {
            const data = res.data
            ...
            return hasil = logger.error
        })

        res.status(hasil.code).json(hasil)
})
router.get('/downloader/ytmp3', async (req, res, next) => {
    ...
    await axios.get('https://saipulanuar.ga/api/download/ytmp3?url=' + url)
        .then((res) => {
            const data = res.data
